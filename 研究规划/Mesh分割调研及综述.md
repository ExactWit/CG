...
| **<br><br>数据集<br><br>** | **<br><br>年份<br><br>** | **<br><br>表示类型<br><br>**                 | **<br><br>标注粒度<br><br>**                                                 | **<br><br>规模<br><br>**     | **<br><br>主要用途/定位<br><br>**                 | 备注          |
| ----------------------- | ---------------------- | ---------------------------------------- | ------------------------------------------------------------------------ | -------------------------- | ------------------------------------------- | ----------- |
| **PartNet**​            | CVPR 2019              | 三角网格                                     | 部件级（面级标签），24 大类                                                          | 26,000+ 实例                 | 监督 mesh part segmentation **主基准**​          | 需要申请授权      |
| **ShapeNet-Part**​      | 2015                   | 三角网格                                     | 部件级（较粗），16 类                                                             | ~16,000 模型                 | 经典对照 / 预训练来源                                | 无           |
| **COSEG**​              | 2012                   | 三角网格                                     | 部件级，少量类别（吉他、椅子等）                                                         | 少量                         | 跨类泛化性测试                                     | 已有椅子、花瓶、外星人 |
| **SHREC 人体部位**​         | 2007                   | 人体扫描网格                                   | 身体部件（头/躯干/四肢）                                                            | 中等                         | 人体分割评估                                      | Human       |
| **Objaverse-XL**​       | 2023                   | 三角网格 / glTF                              | **无部件标注**；仅有类别/元数据                                                       | ~1,000,000 对象              | 3D 生成/检索/原料仓（非分割基准）                         | 非分割任务       |
| **MeshFleet**​          | 2024–25                | 三角网格（通过 Objaverse 拉取）                    | 车辆类别标注 + 多视角渲染；**无部件分割标注**​                                              | 子集（车辆类）                    | Objaverse-XL 车辆子集封装，用于生成/外观评测               | 非分割任务       |
| **Fusion 360 Gallery**​ | 2021（Autodesk AI Lab）  | **B-Rep（Parasolid 拓扑面）、mesh、pointcloud** | 面级类型标注（17 类：平面/圆柱面/螺纹面…）；**另有草图、装配序列、参数化历史等附加数据，segmentation 是其中最常用的子集** | ~8,600 零件（segmentation 子集） | **CAD/B-Rep 面分割权威基准**（UV-Net / BRepNet 的实验台 | 三种表示都有，已下载  |
